...
def train(args, train_dataset, eval_dataset, model, tokenizer, frame_dataset=None):
    ...
    logger.info("Getting best performance in epoch %d, best_dev_acc: %s at step: %s", best_epoch, best_dev_acc,
                best_steps)
...
def test(args, test_dataset, model, frame_dataset=None, model_path=None, tokenizer=None):
    ...
    if args.do_test and model_path is not None:
        writer = csv.writer(
            open(os.path.join(model_path, "test_result_" + args.test_data_mode + ".csv"), "w", encoding="utf-8"))
        writer.writerow(["id", "sentence", "target", "predict", "label", "predict_prob"])
        for id, (sentence, st, ed, predict, label) in enumerate(
                zip(samples, target_start, target_end, preds, out_label_ids)):
    ...
    result = {"test_acc": acc, "preds": preds, "labels": out_label_ids, "target_embeds": target_embeds,
              "frame_embeds": frame_embed}
...
def main():
    ...
    if args.train_data_mode not in data_mode or args.test_data_mode not in data_mode:
        raise ValueError("data_mode: %s %s not in list of %s".format(args.train_data_mode, args.test_data_mode,
                                                                     ", ".join(data_mode)))
