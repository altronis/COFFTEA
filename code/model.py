...
class BertForFrameId(BertPreTrainedModel):
    ...
    def forward(
        ...
        if n_choices is not None:  # map each sentence to corresponding frame def
            ...
            sentence_attention_mask = remove_zero_vectors(sentence_attention_mask,
                                                          n_choices) if sentence_attention_mask is not None else None
            sentence_token_type_ids = remove_zero_vectors(sentence_token_type_ids,
                                                          n_choices) if sentence_token_type_ids is not None else None
            sentence_position_ids = remove_zero_vectors(sentence_position_ids,
                                                        n_choices) if sentence_position_ids is not None else None
            target_start_pos = remove_zero_vectors(target_start_pos,
                                                   n_choices) if target_start_pos is not None else None
            ...
            frame_attention_mask = remove_zero_vectors(frame_attention_mask,
                                                       n_choices) if frame_attention_mask is not None else None
            frame_token_type_ids = remove_zero_vectors(frame_token_type_ids,
                                                       n_choices) if frame_token_type_ids is not None else None
            frame_position_ids = remove_zero_vectors(frame_position_ids,
                                                     n_choices) if frame_position_ids is not None else None
            ...
            reshaped_similarity = torch.zeros([n_batch, max_choice], dtype=similarity.dtype,
                                              device=similarity.device) - float("Inf")
            ...
            if labels is not None:
                loss = sum(F.cross_entropy(similarity[flat_choices[i]:flat_choices[i + 1]].unsqueeze(0),
            ...
            if labels is not None:
                assert self.config.train_mode == "contrastive_learning", KeyError(
                    "Loss func {} not exists".format(self.config.train_mode))
